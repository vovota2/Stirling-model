...
def generate_alpha_engine_animation(alpha_deg):
    ...
    # Základní geometrie hřídele a válců (ZVĚTŠENÝ ZDVIH A MENŠÍ MRTVÝ OBJEM)
    xc = 95 * scale
    yc = 40 * scale
    R = 18 * scale         # Větší klika (původně 15)
    L = 74 * scale         # Delší ojnice (původně 60)
    cyl_w = 34 * scale
    d_base = 40 * scale    # Zvednutý spodek válců
    d_top = 110 * scale
    pipe_w = 12 * scale
    
    # VÝMĚNÍKY POSUNUTY K HORNÍMU OKRAJI (oba stejně, jen kousek od d_top)
    hot_offset = 5 * scale   
    cold_offset = 5 * scale  
    hot_h = 30 * scale       
    ...
    # Setrvačník (zvětšen pro větší kliku)
    fw_R = 26 * scale
    ...
    pist_pts = [
        ...
        (-w/2, p_top), (w/2, p_top),
        (w/2, p_mid), (stopka_w/2, p_mid)
    ]
    
    pist_H_poly = patches.Polygon(pist_pts, facecolor=c_pist, edgecolor=c_line, lw=lw, transform=trans_H, zorder=6)
    pist_C_poly = patches.Polygon(pist_pts, facecolor=c_pist, edgecolor=c_line, lw=lw, transform=trans_C, zorder=6)
...
with tab4:
    # --- 1. Graf kumulovaneho tepla v Joulech (Puvodni) ---
    fig = go.Figure()
    ...
    # --- 2. NOVY Graf: Okamzity tepelny tok regeneratorem ve Wattech ---
    # Okamzity vykon P = dQ/dt = (dQ/dφ) * (dφ/dt) = (dQ/dφ) * 2πf
    omega = 2 * np.pi * lp['f']
